...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" queryBinding="xslt2">
    <!-- 
    Ensure SIGMET Observation information is of the correct type.  
    ...
    <sch:pattern>
        ...
        <sch:rule context="//iwxxm:SIGMET">
            <sch:assert test="iwxxm:analysis/om:OM_Observation/om:result/iwxxm:EvolvingMeteorologicalCondition">
                Result should be a EvolvingMeteorologicalCondition
            </sch:assert>
            <sch:assert test="iwxxm:analysis/om:OM_Observation/om:featureOfInterest/sams:SF_SpatialSamplingFeature/sf:type[@xlink:href='http://www.opengis.net/def/samplingFeatureType/OGC-OM/2.0/SF_SamplingSurface']">
                featureOfInterest should be an SF_SamplingPoint (type is an xlink:href to http://www.opengis.net/def/samplingFeatureType/OGC-OM/2.0/SF_SamplingSurface)
            </sch:assert>
        </sch:rule>
        ...
        <sch:rule context="//iwxxm:TropicalCycloneSIGMET">
            ...
            <sch:assert test="empty( iwxxm:forecastPositionAnalysis ) or iwxxm:forecastPositionAnalysis/om:OM_Observation/om:result/iwxxm:MeteorologicalFeatureCollection">
                Result should be a MeteorologicalFeatureCollection
            </sch:assert>
            <!-- Removed - better not to have this test than to have a brittle test requiring multiple xlink type tests
            <sch:assert test="empty( iwxxm:forecastPositionAnalysis ) or iwxxm:changeForecast/om:OM_Observation/om:featureOfInterest/sams:SF_SpatialSamplingFeature/sf:type[@xlink:href='http://www.opengis.net/def/samplingFeatureType/OGC-OM/2.0/SF_SamplingSurface']">
                featureOfInterest should be an SF_SamplingPoint (type is an xlink:href to http://www.opengis.net/def/samplingFeatureType/OGC-OM/2.0/SF_SamplingSurface)
            </sch:assert>
            -->
        </sch:rule>
