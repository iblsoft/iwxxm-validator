<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
   ...
   <sch:pattern id="METCE.MC1">
      <sch:rule context="//metce:MeasurementContext">
         <sch:assert test="((: TO BE IMPLEMENTATED WHEN REPOSITORY IS READY :) true())">METCE.MC1: unitOfMeasure shall be appropriate for measurand</sch:assert>
      </sch:rule>
   </sch:pattern>
   <sch:pattern id="METCE.MC2">
      <sch:rule context="//metce:MeasurementContext">
         <sch:assert test="(if( exists(metce:measuringInterval) or exists(metce:resolutionScale) ) then ( exists(metce:unitOfMeasure) ) else( true() ))">METCE.MC2: if measuringInterval or resolutionScale or both are given then
   ...
   <sch:pattern id="METCE.RB1">
      <sch:rule context="//metce:RangeBounds">
         <sch:assert test="(number( metce:rangeStart/text() ) lt number( metce:rangeEnd/text() ))">METCE.RB1: The extreme lower limit of the range of interval must be less than the
            extreme upper limit.
         </sch:assert>
      </sch:rule>
   </sch:pattern>
